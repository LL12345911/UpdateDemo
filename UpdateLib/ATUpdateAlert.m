...
+ (void)showUpdateAlertWithVersion:(NSString *)version Descriptions:(NSArray *)descriptions{
    ...
    NSLog(@"====%@",description);
    ATUpdateAlert *updateAlert = [[ATUpdateAlert alloc]initVersion:version description:description];
...
+ (void)showUpdateAlert:(ATAppInfo *)appInfo{
    ATUpdateAlert *updateAlert = [[ATUpdateAlert alloc]initVersion:appInfo.version description:appInfo.releaseNotes];
    updateAlert.trackViewUrl = appInfo.trackViewUrl;
    [[UIApplication sharedApplication].delegate.window addSubview:updateAlert];
}
...
- (void)_setupUI{
    ...
    NSString * path = [[NSBundle mainBundle] pathForResource:@"Update" ofType:@"bundle"];
   ...
   // updateIcon.image = [UIImage imageNamed:@"VersionUpdate_Icon"];
    ...
    versionLabel.text = [NSString stringWithFormat:@"发现新版本 %@",self.version];
